...
"""
날짜 오류 수정 스크립트 Final
모든 날짜 입력 오류를 정확히 수정 (8개 패턴 + 특수 문자 오류)
"""
...
for record in records:
    ...
    # 패턴 6: 2022-10??3 1028 → 2022-10-03 (특수문자 또는 깨진 한글)
    # 2022-10 뒤에 이상한 문자가 있고 3이 나오며 공백 후 1028이 나오는 패턴
    elif re.match(r'^2022-10.{1,3}3\s+1028', original_date):
        record['사용일시'] = '2022-10-03'
        print(f"[패턴6] {repr(original_date):30s} -> {record['사용일시']}")
        fixed = True
...
if strange_dates:
    ...
    for r in strange_dates[:10]:
        print(f"  {repr(r['사용일시'][:30]):35s} | {int(r['사용금액']):>10,}원 | {r['집행목적'][:30]}")
else:
